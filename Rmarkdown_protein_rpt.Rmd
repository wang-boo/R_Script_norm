...
for (go_ppi in up.vs.dn) {
  ...
  if (any(sample.list$replicates < 3)) {
    source('Y:/R_Script_norm/v201801//fc-plot.R', echo = TRUE)
  } else{
    source('Y:/R_Script_norm/v201801/cluster.R', echo = TRUE)
...
for (go_ppi in up.vs.dn) {
  ...
  if (up.dn) {
    ...
    source('Y:/R_Script_norm/v201801//GO&KEGG.R', echo = TRUE)
  }
}
#### NETWORK 2 ####
for (go_ppi in up.vs.dn) {
  source('Y:/R_Script_norm/v201801//setwd.R', echo = TRUE)
  source('Y:/R_Script_norm/v201801//PPI.R', echo = TRUE)
}
```

```{r collating_files, include=FALSE}
  #in report dir
  copy_remove_file <- function(from,to){
  ...
  for (one_folder in  c("QC","raw_data", "summary", "cluster")){
    dir.create(one_folder)
  }
  copy_remove_file(c("correlation.pdf","correlation.tif", "rank.pdf", "rank.tiff", "TIC.pdf"), "QC")
  copy_remove_file(c("cluster.tiff", "cluster.pdf"), "cluster")
  copy_remove_file(c("REPORT SUMMARY.txt", "config.list", "sample.list"), "summary")
  #file.copy(c("REPORT SUMMARY.txt", "config.list", "sample.list"), "raw_data")
  
  ## in compare dir 
  for (compare in up.vs.dn){
    print(compare)
    setwd(paste(dir_1, compare, sep="/"))
    for(one_folder in c("foldchange", "GO&KEGG")){
      dir.create(one_folder)
    }
    c_files <- c("GO-all.h.pdf", "GO-all.h.tiff", "GO-all.v.pdf", "GO-all.v.tiff", "KEGG.pdf", "KEGG.tiff",
                 paste("enrichGO.", compare, ".all.csv",sep=""),
                 paste("enrichGO.", compare, ".BP.csv",sep=""),
                 paste("enrichGO.", compare, ".CC.csv",sep=""),
                 paste("enrichGO.", compare, ".MF.csv",sep=""),
                 paste("enrichKEGG.", compare, ".csv",sep="")
                )
    copy_remove_file(c_files, "GO&KEGG")
    copy_remove_file(c("FC.pdf", "FC.tiff"), "foldchange")
  }
```
...
如下图:Label-free定量技术原理 


```{r}
  knitr::include_graphics(path="Y:/Labelfree.tif")
```


```{r}
  #load("Y:/WD_1711125/combined/txt/RUN-0.1/sample.list")
  #attach(sample.list)
  vs <- sample.list$up.vs.dn[1]
  #dir_1 = "Y:/WD_1711125/combined/txt/RUN-0.1/"
```
...
如下图:蛋白丰度分布图
```{r}
  rank_path <- paste(dir_1,"QC","rank.tiff",sep="/")
...
下图展示了`r length(sample.list$bioID)`个样品总离子强度（[TIC](QC/TIC.pdf)）及色谱保留时间（RT）的稳定性，TIC和RT的稳定性是label-free定量可靠性的前提保证。  
...
如下图：总离子流图(TIC)
```{r}
  TIC_path <- paste(dir_1, "QC","TIC.pdf", sep = "/")
  knitr::include_graphics(path= TIC_path)
```
  
如下图：`r length(sample.list$bioID)`个样本间定量重复性分析
```{r}
  correlation_path <- paste(dir_1, "QC", "correlation.pdf", sep = "/")
  knitr::include_graphics(path= correlation_path)
```

## 差异倍数展示
由于无生物学重复，无法进行火山图展示，只能进行差异倍数的展示。[附件：FC](`r paste(sample.list$up.vs.dn[1], "foldchange", "FC.tiff", sep="/")`) 
  
如下图：`r vs`蛋白差异倍数展示。差异显著的蛋白分别以深绿色（下调）和橙红色（上调）表示。无统计学差异蛋白以淡绿色表示。

  
```{r} 
  fc_path = paste(dir_1, vs, "foldchange", "FC.tiff",sep="/")
  knitr::include_graphics(path=fc_path)
```
```{r volcano, eval = FALSE}
  vol <- "##火山图展示
对差异表达的蛋白进行火山图展示，y轴：-log10(p-value)；x轴：log2(ratio)；分布在两条纵向边界线之外、横向边界之上的点代表差异显著的蛋白，并以symbol值作为标注。附件：v-plot"
  vol
  #fc_path = paste(dir_1, vs, "volcano.tiff",sep="/")
  #knitr::include_graphics(path=fc_path)
```
...
# 生信分析  
## 聚类分析  
为检验所选取的差异表达蛋白质或特征差异表达蛋白质的合理性和准确性，我们利用筛选出的蛋白质，对各组样本进行层次聚类。原始定量值先进行Z-score变换后再进行聚类分析和展示。(R-package: ComplexHeatmap)  [*附件：cluster*](cluster/cluster.tiff)
  
如下图：差异蛋白层次聚类分析，顶端颜色条代表样品分组情况（例如，实验组、对照组，对应图例：bio.group）；低端为对应样品名称；右侧图例Z-score以颜色代表蛋白的相对表达水平；右侧蛋白名对应于差异蛋白清单（significant.csv）中symbol列。
```{r}
  cluster_path <- paste(dir_1, "cluster", "cluster.tiff", sep = "/")
...
如下图:`r vs`差异蛋白GO富集分析。Bar: -log( p.adjust), point: Count。
```{r}
  go_path = paste(dir_1, vs, "GO&KEGG","GO-all.v.tiff", sep="/")
  if(file.exists(go_path)){
  knitr::include_graphics(path=go_path)
  }else{
    print("no GO-all.v.tiff, please try another compare")
  }
```
...
如下图：`r vs`差异蛋白KEGG富集分析。Bar: -log( p.adjust), point: Count
```{r KEGG}
  kegg_path = paste(dir_1, vs, "GO&KEGG", "KEGG.tiff", sep="/")
  if(file.exists(kegg_path)){
  knitr::include_graphics(path=kegg_path)
  }else{
    print("no KEGG.tiff, please try another compare")
  }
```  
...
如下图:STRING 蛋白互作分析。运用STRINGdb对`r vs`差异表达的蛋白进行蛋白互作分析发现，213个基因间存在显著富集的相互作用，PPI enrichment p-value :0。
Known Interactions
...
```{r}
  ...
  knitr::include_graphics(path=PPI_path)
    ...
    print("no PPI.tiff, please try another compare")
  }
```

```{r fig.width=2}
  explain_path = "Y:/interaction_explain.png"
  knitr::include_graphics(path=explain_path)
```
...
```
```{r include=FALSE}
 file.copy("Y:/R_Script_norm/v201801/Rmarkdown_protein_rpt.pdf", dir_1)
```
